...
def repo_root(path: str | Path) -> Path:
    ...
    if not (root / "library").is_dir():
        raise SystemExit(f"repo root not found or missing library/: {root}")
    return root
...
def term_records(root: Path) -> list[dict[str, Any]]:
    return load_jsonl(root / "library" / "concepts" / "term-senses.jsonl")


def relation_records(root: Path) -> list[dict[str, Any]]:
    return load_jsonl(root / "library" / "relations" / "relation-assets.jsonl")


def segment_records(root: Path) -> list[dict[str, Any]]:
    return load_jsonl(root / "library" / "manifests" / "segments.jsonl")
