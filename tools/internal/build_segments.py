#!/usr/bin/env python3
"""Build segment cards in bounded batches.

Generate draft segment cards directly from corpus manifest segment manifests + clean markdown.
...
def build_index(segments: list[dict], card_rows: set[int], processed_rows: list[int]) -> None:
    ...
    for seg in segments:
        ...
        rel = f"[{p.name}]({p.name})" if status == "draft" else "-"
...
def update_state(batch_rows: list[int]) -> None:
    """Status updates are intentionally kept out of product docs.

    Segment card generation is a local maintenance operation; durable product
    state is represented by validators and manifests, not by appending workflow
    notes to docs/status.md.
    """
    return

def log(msg: str) -> None:
    """Intentionally no-op: validation artifacts, not process logs, are durable state."""
    return
...
def main() -> None:
    ...
    msg = (
        f"Generated segment batch size={len(processed)} from row {start_row}; "
        f"rows={','.join(map(str, processed)) if processed else 'none'}. "
        "Updated library/segments/index.md."
    )
    log(msg)
