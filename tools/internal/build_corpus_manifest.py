#!/usr/bin/env python3
"""Build corpus manifests for the product-facing ISMISM library.

The registry source is `corpus/registry/toc.csv`; text sources are
`corpus/raw-markdown/` and `corpus/clean-markdown/`; source PDF slices live in
`corpus/pdf-slices/` when regenerated. Only source and cleaned text layers are
modeled as canonical corpus inputs.
"""
...
KNOWLEDGE = ROOT / "library"
MANIFESTS = KNOWLEDGE / "manifests"
CSV_PATH = ROOT / "corpus/registry/toc.csv"
SOURCE_PDF_NAME = "corpus/source/ismism.pdf"
SOURCE_PDF = ROOT / SOURCE_PDF_NAME
PDF_SLICES_DIR = ROOT / "corpus/pdf-slices"
RAW_FIELD = "corpus/raw-markdown_relpath"
RAW_EXISTS_FIELD = "corpus/raw-markdown_exists"
...
def build_segments(rows: List[Dict[str, str]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for row in rows:
        ...
        raw_rel = row_value(row, RAW_FIELD, "raw_markdown_relpath")
        clean_rel = raw_rel.replace("corpus/raw-markdown/", "corpus/clean-markdown/", 1)
        pdf_rel = row_value(row, "pdf_slice_relpath")
        raw_path = ROOT / raw_rel
        clean_path = ROOT / clean_rel
        pdf_slices_path = ROOT / pdf_rel if pdf_rel else None

        raw_exists = raw_path.exists() and raw_path.is_file()
        clean_exists = clean_path.exists() and clean_path.is_file()
        pdf_slices_file_exists = bool(pdf_slices_path and pdf_slices_path.exists() and pdf_slices_path.is_file())
        pdf_slices_dir_exists = PDF_SLICES_DIR.exists()
        ...
        if not pdf_slices_dir_exists:
            # This is corpus-level derived-layer absence, not segment-level text failure.
            pass
        elif not pdf_slices_file_exists:
            missing_reasons.append("pdf_slices_file_missing")
        ...
        record: Dict[str, Any] = {
            ...
            "source_paths": {
                "csv_corpus/raw-markdown_relpath": raw_rel,
                "raw_md_relpath": raw_rel,
                "clean_md_relpath": clean_rel,
                "pdf_slice_relpath": pdf_rel,
                "source_pdf_relpath": SOURCE_PDF_NAME,
            },
            "existence": {
                "raw_md": raw_exists,
                "clean_md": clean_exists,
                "pdf_slices_dir": pdf_slices_dir_exists,
                "pdf_slices_file": pdf_slices_file_exists,
                "source_pdf": source_pdf_exists,
                "csv_corpus/raw-markdown_exists_flag": row_value(row, RAW_EXISTS_FIELD, "raw_markdown_exists"),
                "csv_pdf_slice_exists_flag": row.get("pdf_slice_exists"),
            },
        ...
        if truthy(row_value(row, RAW_EXISTS_FIELD, "raw_markdown_exists")) and not raw_exists:
            anomalies.append({
                "type": "stale_csv_flag",
                "severity": "warning",
                "field": RAW_EXISTS_FIELD,
                "csv_value": row_value(row, RAW_EXISTS_FIELD, "raw_markdown_exists"),
        ...
        if truthy(row.get("pdf_slice_exists")) and not pdf_slices_file_exists:
            anomalies.append({
                "type": "stale_csv_flag",
                "severity": "expected_due_to_absent_pdf_slices_dir" if not pdf_slices_dir_exists else "warning",
                "field": "pdf_slice_exists",
                "csv_value": row.get("pdf_slice_exists"),
                "actual_value": pdf_slices_file_exists,
                "segment_id": record["segment_id"],
                "row_id": row_id,
                "toc_id": record["toc_id"],
                "title": record["title"],
...
def build_manifest(rows: List[Dict[str, str]], segments: List[Dict[str, Any]], chunks: List[Dict[str, Any]], anomalies: List[Dict[str, Any]]) -> Dict[str, Any]:
    raw_actual = sorted(str(p.relative_to(ROOT)) for p in (ROOT / "corpus/raw-markdown").rglob("*.md")) if (ROOT / "corpus/raw-markdown").exists() else []
    clean_actual = sorted(str(p.relative_to(ROOT)) for p in (ROOT / "corpus/clean-markdown").rglob("*.md")) if (ROOT / "corpus/clean-markdown").exists() else []
    ...
    return {
        ...
        "repository_root": ".",
        "objective": "Build row, text-source, anomaly, and retrieval-chunk manifests for the ISMISM Library.",
        "source_truth_layers": {
            "primary": ["corpus/registry/toc.csv", "corpus/raw-markdown/", "corpus/clean-markdown/", SOURCE_PDF_NAME],
            "derived_absent": ["corpus/pdf-slices/"],
        },
        "source_files": {
        ...
        "directory_inventory": {
            "corpus/raw-markdown_file_count": len(raw_actual),
            "corpus/clean-markdown_file_count": len(clean_actual),
            "pdf_slices_dir_exists": PDF_SLICES_DIR.exists(),
            "pdf_slices_file_count": count_files(PDF_SLICES_DIR),
        },
        "counts": {
            "toc_rows": len(rows),
        ...
        "decisions": {
            ...
            "clean_md_path_scheme": "replace leading corpus/raw-markdown/ with corpus/clean-markdown/ from CSV corpus/raw-markdown_relpath",
            "segment_text_status": "available only when both raw_md and clean_md exist; pdf_slices absence is corpus-level derived-layer absence, not segment text failure",
        ...
        "artifacts": {
            "segments_jsonl": "library/manifests/segments.jsonl",
            "chunks_jsonl": "library/manifests/chunks.jsonl",
            "corpus_manifest_json": "library/manifests/corpus-manifest.json",
            "missing_and_anomalies_md": "library/manifests/missing-and-anomalies.md",
            "build_script": "tools/internal/build_corpus_manifest.py",
        },
    }
...
def write_anomalies_md(path: Path, manifest: Dict[str, Any], anomalies: List[Dict[str, Any]]) -> None:
    ...
    lines: List[str] = []
    lines.append("# Corpus Manifest Missing and Anomalies Report")
    lines.append("")
    ...
    lines.append(f"- actual `corpus/raw-markdown/*.md`: **{manifest['directory_inventory']['corpus/raw-markdown_file_count']}**")
    lines.append(f"- actual `corpus/clean-markdown/*.md`: **{manifest['directory_inventory']['corpus/clean-markdown_file_count']}**")
    lines.append(f"- `corpus/pdf-slices/` directory exists: **{manifest['directory_inventory']['pdf_slices_dir_exists']}**")
    lines.append("")
    ...
    if missing:
        ...
        lines.append("")
    pdf_slices_present = manifest["directory_inventory"]["pdf_slices_dir_exists"]
    pdf_slices_count = manifest["directory_inventory"]["pdf_slices_file_count"]
    lines.append(f"## Derived Layer Status: `corpus/pdf-slices/` {'present' if pdf_slices_present else 'absent'}")
    lines.append("")
    if pdf_slices_present:
        lines.append(f"`corpus/pdf-slices/` is present with **{pdf_slices_count}** files. It remains a regenerable PDF-slice derived layer, not an interpretation source and not a raw/clean text source. `pdf_slice_exists` in the CSV should reflect actual filesystem state.")
    else:
        lines.append("`corpus/pdf-slices/` is intentionally absent in this repository after project cleanup. It remains a regenerable PDF-slice derived layer, not an interpretation source and not a raw/clean text source. `pdf_slice_exists` in the CSV should reflect actual filesystem state; rows stay `0` unless the PDF slices are explicitly regenerated.")
    lines.append("")
    lines.append("## Stale CSV Existence Flags")
    lines.append("")
    stale_md = [a for a in anomalies if a["type"] == "stale_csv_flag" and a["field"] == "corpus/raw-markdown_exists"]
    stale_pdf = [a for a in anomalies if a["type"] == "stale_csv_flag" and a["field"] == "pdf_slice_exists"]
    lines.append(f"- stale `corpus/raw-markdown_exists` flags: **{len(stale_md)}**")
    lines.append(f"- stale `pdf_slice_exists` flags: **{len(stale_pdf)}**")
    lines.append("")
    if stale_md:
        lines.append("### Stale `corpus/raw-markdown_exists` rows")
        lines.append("")
        for a in stale_md:
    ...
    lines.append("")
    lines.append("Use `segments.jsonl` as the structural registry. Do not treat `corpus/pdf-slices/` absence as a missing text segment. Row 176 / `2-4-2-4` remains available through raw/clean Markdown; regenerate PDF slices only if a future task explicitly needs that derived layer.")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    print(json.dumps({
        ...
        "chunks": len(chunks),
        "pdf_slices_dir_exists": manifest["directory_inventory"]["pdf_slices_dir_exists"],
