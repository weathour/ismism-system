#!/usr/bin/env python3
"""
Validate relation relation assets against the current corpus/position evidence layers.
...
- source_position and target_position point to existing position position cards.
- every evidence_segment quote is an exact substring of its row's corpus/clean-markdown text.
- term:*:sNN sources/targets/source_senses point to existing concept term sense IDs.

Use --min-count 60 --require-type-min 2 for the final project knowledge contract relation gate.
Use defaults for an in-progress relation integrity check.
"""
...
def load_sense_ids(repo: Path) -> set[str]:
    ...
    path = repo / "library/concepts/term-senses.jsonl"
    if not path.exists():
        return sense_ids
    with path.open(encoding="utf-8") as f:
        for line in f:
...
def term_ref_to_sense_id(ref: Any) -> str | None:
    """Convert term:<term>:sNN relation refs to term:<term>:sNN concept sense IDs."""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Validate relation relation-assets.jsonl evidence.")
    ap.add_argument("--repo", default=".", help="repository root")
    ap.add_argument(
        "--relations",
        default="library/relations/relation-assets.jsonl",
    ...
    with path.open(encoding="utf-8") as f:
        for lineno, line in enumerate(f, 1):
            ...
            for pos_field in ("source_position", "target_position"):
                ...
                if pos and not (repo / f"library/positions/{pos}.md").exists():
    ...
    if args.json:
        ...
        print(
            f"relation validation: records={checked_records}, quotes={checked_quotes}, "
