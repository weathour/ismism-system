#!/usr/bin/env python3
"""
Generate concept candidate-only term sense material with a local Ollama model.

This script is deliberately NOT allowed to update formal concept files such as:
- library/concepts/term-senses.jsonl
- library/concepts/core-terms.md
- library/concepts/ambiguous-terms.md

It only writes candidate files under library/concepts/candidates/ and a run log.
...
def build_prompt(term: str, contexts: list[dict[str, Any]], max_quotes: int) -> str:
    ...
    return f"""你是 ISMISM concept 术语候选预处理低模型，只做 evidence mining 和粗聚类。
...
def write_candidate_md(path: Path, term: str, batch_id: str, contexts: list[dict[str, Any]], items: list[dict[str, Any]], meta: dict[str, Any]) -> None:
    lines: list[str] = []
    lines.append(f"# concept Candidate Senses — {term}\n")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Generate concept candidate-only term evidence with Ollama.")
    ap.add_argument("--repo", default=".", help="repo root")
    ap.add_argument("--terms", nargs="*", default=DEFAULT_TERMS)
    ap.add_argument("--model", default=DEFAULT_MODEL)
    ap.add_argument("--api-url", default=DEFAULT_API)
    ap.add_argument("--batch-id", default="concept-B6-candidates")
    ...
    segments = load_segments(repo / "library/manifests/segments.jsonl", repo)
    stats = load_stats(repo / "library/concepts/term-candidate-stats.tsv")
    out_dir = repo / "library/concepts/candidates"
