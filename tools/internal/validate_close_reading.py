#!/usr/bin/env python3
"""Validate close-reading close-reading pilot cards.

The validator intentionally checks only local repository evidence. It treats
close-reading as an additive draft layer and verifies that every card remains anchored to
corpus manifest segment metadata and exact substrings in corpus/clean-markdown.
"""
...
CARD_TYPES = {
    "close-reading-argument": {"dir": "arguments", "id_prefix": "cr:arg:", "file_prefix": "argument-"},
    "close-reading-process": {"dir": "processes", "id_prefix": "cr:proc:", "file_prefix": "process-"},
    "close-reading-case": {"dir": "cases", "id_prefix": "cr:case:", "file_prefix": "case-"},
}
...
def load_segments(repo: Path) -> dict[int, dict[str, Any]]:
    segments_path = repo / "library/manifests/segments.jsonl"
...
def collect_repo_cards(repo: Path) -> tuple[list[Path], list[str]]:
    close_reading_root = repo / "library/close-reading"
    paths: list[Path] = []
    errors: list[str] = []
    allowed_docs = {close_reading_root / "README.md", close_reading_root / "index.md"}
    allowed_dirs = {close_reading_root / spec["dir"] for spec in CARD_TYPES.values()}
    for markdown_path in sorted(close_reading_root.rglob("*.md")):
        ...
        if markdown_path.parent in allowed_dirs:
            ...
            continue
        errors.append(
            f"{markdown_path}: unexpected close_reading markdown; only README.md, index.md, "
            "and cards in arguments/processes/cases are allowed"
        )
...
def parse_index_entries(repo: Path, index_path: Path, index_text: str) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for line_number, raw in enumerate(index_text.splitlines(), 1):
        ...
        if not in_pilot_table:
            if line.startswith("|") and "cr:" in line:
                errors.append(f"index.md:{line_number}: close-reading card row appears outside the Pilot cards table")
            continue
        if not line:
            in_pilot_table = False
            continue
        ...
        if len(cells) != 5:
            errors.append(f"index.md:{line_number}: malformed close-reading index row; expected 5 cells")
            continue
    ...
    for entry in entries:
        ...
        if key in seen:
            errors.append(f"index.md:{entry['line_number']}: duplicate close-reading index entry {key}")
        seen.add(key)
...
def validate_index_exact(expected_entries: list[dict[str, Any]], index_entries: list[dict[str, Any]]) -> list[str]:
    ...
    for missing in sorted(expected - actual):
        errors.append(f"index.md: missing exact close-reading index entry {missing}")
    for stale in sorted(actual - expected):
        errors.append(f"index.md: stale or mismatched close-reading index entry {stale}")
    return errors
...
def validate_card(repo: Path, segments: dict[int, dict[str, Any]], card_path: Path, seen_ids: set[str]) -> tuple[dict[str, Any] | None, list[str]]:
    ...
    if card_type in CARD_TYPES:
        ...
        close_reading_root = repo / "library/close-reading"
        if is_under(card_path, close_reading_root):
            expected_dir = close_reading_root / spec["dir"]
    ...
    gap_review = str(meta.get("concept_relation_gap_review", ""))
    if gap_review not in ACCEPTED_GAP_REVIEW:
        errors.append(f"{card_path}: invalid concept_relation_gap_review {gap_review!r}")
    ...
    if isinstance(quotes, list) and quotes:
        ...
        if "限定本行资本主义功能的一个环节" in substantive_body:
            errors.append(f"{card_path}: Capitalism close-reading body still contains placeholder close_reading text")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate close-reading cards")
    ...
    errors: list[str] = []
    close_reading_root = repo / "library/close-reading"
    plan_path = close_reading_root / "README.md"
    index_path = close_reading_root / "index.md"
    if not plan_path.exists():
        errors.append("missing library/close-reading/README.md")
    if not index_path.exists():
        errors.append("missing library/close-reading/index.md")
    index_text = index_path.read_text(encoding="utf-8") if index_path.exists() else ""

    try:
        segments = load_segments(repo)
    except Exception as exc:  # pragma: no cover - defensive CLI surface
        print(f"validate_close_reading: failed to load segments.jsonl: {exc}", file=sys.stderr)
        return 2
    ...
    for card_path in card_paths:
        if not card_path.exists():
            ...
            continue
        is_repo_card = is_under(card_path, close_reading_root)
