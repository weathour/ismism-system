...
- 中文名：现实社会现象诊断协议
- status: shared method protocol for the Social Phenomena / Everyday Life Problems Maximum Absorption Superphase
...
This protocol defines the common method for the next maximum absorption superphase: **现实社会现象与问题最大吸收超阶段**.
...
| `<theme>-w3-w5-batch-notes.md` | W3/W5 batch ids, relation boundaries, duplicate-control notes. |
| `<theme>-...-synthesis.md` | Evidence-marker syntheses. |
| `<THEME>-MAXIMUM-ABSORPTION-HANDOFF.md` | Resume state, counts, validation commands, boundaries. |

Companion scripts:

| Script | Function |
|---|---|
| `knowledge/scripts/validate_<theme>_theme.py` | Final validator for manifest/evidence/W3/W5/W10/docs markers. |
| `knowledge/scripts/query_<theme>_theme.py` | Query helper for keywords, rows, classes, and evidence ids. |
...
The quote must be an exact substring of the declared `split_md_clean/` file after the validator's established whitespace normalization.
...
## 6. W3 term-sense protocol

W3 additions for social-phenomenon themes must remain `draft`.

Add a new W3 sense only when one of these is true:
...
- Search existing `knowledge/lexicon/term-senses.jsonl` before appending.
...
- Batch ids should be theme-specific and date-stamped, for example `W3-LABOR-WORKPLACE-PRECARITY-2026-06-16` or execution date.

## 7. W5 relation protocol

W5 additions must remain `draft` and use existing relation type discipline.
...
If no existing W5 relation type fits, do not invent a type casually. Use the nearest existing relation type and explain the choice in batch notes.

## 8. W10 close-reading protocol

W10 additions remain `pilot-draft`.

A social-phenomenon W10 card should do one of three things:
...
Each W10 card should include the diagnostic grammar where possible and cite exact row evidence. Avoid writing cards that merely summarize a row without social diagnostic function.
...
The final superphase query layer should test at least 30 prompts, including:
...
4. W3 batch exists and all records are `draft`,
5. W5 batch exists and all records are `draft`,
6. W10 theme cards exist and remain `pilot-draft`,
7. README, synthesis, handoff, query helper, and navigation markers are present,
8. global current counts and W5 min-count are not stale,
9. protected corpus directories have no diff.

Superphase final validator should additionally check:
...
- W3/W5/W10/master validators pass.
...
## 13. Completion standard for the superphase

The superphase is complete when a future user can ask a concrete social question and the repository can return a traceable answer with:

- relevant rows,
- exact quotes,
- W3 draft concepts,
- W5 draft relations,
- W10 pilot-draft close readings,
- one or more theme syntheses,
- one or more cross-theme routes,
- and a clear boundary between evidence and inference.

The final state must pass all theme validators, global W3/W5/W10 validators, Universal-A/B validators, existing theme validators, master validator, query smoke tests, `git diff --check`, and protected corpus diff check.
