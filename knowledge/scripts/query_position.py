#!/usr/bin/env python3
"""Query W4 ISMISM matrix position cards."""
from __future__ import annotations

import argparse
from pathlib import Path
from ismism_query_lib import as_json, front_matter_and_body, repo_root, section_text, truncate


def load_cards(root: Path):
    cards = []
    for path in sorted((root / "knowledge" / "position-cards").glob("*.md")):
...
def print_markdown(matches, root: Path, args) -> None:
    print(f"# W4 position query: {args.query}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Query W4 position cards by coordinate or title text.")
