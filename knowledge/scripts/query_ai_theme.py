...
def main():
    ap=argparse.ArgumentParser(description='Query knowledge/themes/ai AI absorption artifacts')
    ...
    args=ap.parse_args()
    repo=Path(args.repo)
    manifest=list(load_jsonl(repo/'knowledge/themes/ai/ai-row-manifest.jsonl'))
    evidence=list(load_jsonl(repo/'knowledge/themes/ai/ai-evidence-bank.jsonl'))
    ev_by_row={}
    for ev in evidence:
    ...
    for rec in rows[:args.limit]:
        print(f"row {rec['row_id']} {rec['toc_id']} {rec['title']} [{rec['theme_class']}; {rec['core_status']}; {rec['recommended_action']}]")
