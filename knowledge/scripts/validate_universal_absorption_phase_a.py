#!/usr/bin/env python3
"""Validate Universal Absorption Phase A artifacts.
...
- W3/W5 Universal-A batches must be draft and within requested counts;
- Universal-A W10 cards must be pilot-draft and covered by the normal W10 validator shape;
...
GAP_MAP = "knowledge/qa/universal-absorption-phase-a-gap-map.jsonl"
EVIDENCE_BANK = "knowledge/qa/universal-absorption-phase-a-evidence-bank.jsonl"
...
REQUIRED_DOCS_FINAL = [
    GAP_MAP,
    "knowledge/qa/universal-absorption-phase-a-plan.md",
    EVIDENCE_BANK,
    "knowledge/qa/universal-absorption-phase-a-audit.md",
    "knowledge/qa/universal-absorption-phase-a-evidence-claim-audit.md",
    "knowledge/qa/universal-absorption-phase-a-ultraqa-report.md",
    "knowledge/qa/universal-absorption-phase-a-ai-slop-cleaner-report.md",
    "knowledge/qa/universal-absorption-phase-a-code-review-report.md",
    "knowledge/qa/universal-absorption-phase-a-handoff.md",
    "knowledge/STATE.md",
...
def validate_gap_map(repo: Path, segments: dict[int, dict[str, Any]], errors: list[str]) -> set[int]:
    ...
    for lineno, rec in iter_jsonl(path):
        ...
        if rec.get("recommended_action") == "w3+w5+w10" or rec.get("priority_tier") == "target-full-overlap":
            targets.add(row_id)
    if len(seen) != 109:
        add(errors, f"gap-map: expected 109 baseline W1/W2-only rows, found {len(seen)}")
    if len(targets) < 50:
        add(errors, f"gap-map: expected at least 50 target rows, found {len(targets)}")
    return targets
...
def validate_w3(repo: Path, segments: dict[int, dict[str, Any]], target_rows: set[int], errors: list[str]) -> set[str]:
    ...
    if not (80 <= count <= 120):
        add(errors, f"w3: expected 80-120 records for {W3_BATCH}, found {count}")
...
def validate_w5(repo: Path, segments: dict[int, dict[str, Any]], target_rows: set[int], sense_ids: set[str], errors: list[str]) -> set[str]:
    ...
    if not (70 <= count <= 100):
        add(errors, f"w5: expected 70-100 records for {W5_BATCH}, found {count}")
    missing = sorted(target_rows - rows)
    if missing:
...
def validate_w10(repo: Path, segments: dict[int, dict[str, Any]], target_rows: set[int], errors: list[str]) -> set[str]:
    ids: set[str] = set()
    rows: set[int] = set()
    cards = sorted((repo / W10_ROOT).glob("*-cards/*universal-phase-a*.md"))
    for path in cards:
    ...
    if not (50 <= len(cards) <= 70):
        add(errors, f"w10: expected 50-70 Universal-A cards, found {len(cards)}")
    missing = sorted(target_rows - rows)
    if missing:
        add(errors, f"w10: target rows without Universal-A W10 card: {missing[:20]}")
    return ids
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Validate Universal Absorption Phase A artifacts")
    ...
    if args.final:
        ...
        if metrics["w1_w2_only_rows"] > 75:
            add(errors, f"final: W1/W2-only rows {metrics['w1_w2_only_rows']} exceeds conservative target 75")
        if metrics["clean_text_volume_any_pct"] < 82.0:
            add(errors, f"final: any-layer clean volume {metrics['clean_text_volume_any_pct']} below 82%")
        if metrics["full_overlap_rows"] < 150:
            add(errors, f"final: full overlap rows {metrics['full_overlap_rows']} below 150")
        if metrics["w10_rows"] < 185:
            add(errors, f"final: W10 rows {metrics['w10_rows']} below 185")
    ...
    if args.json:
        ...
        print(f"validate_universal_absorption_phase_a: {summary['status']}")
