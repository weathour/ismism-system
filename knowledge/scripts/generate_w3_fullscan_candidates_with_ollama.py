#!/usr/bin/env python3
"""
Heavy full-corpus W3 candidate-only scanner using local Ollama.

This is intentionally a pre-processing miner:
- scans split_md_clean via knowledge/manifests/segments.jsonl
- extracts bounded windows for specified terms
- sends shards to a local model
- writes only to knowledge/lexicon/candidates/fullscan/<batch>/<term>/

It must never update formal W3 registry files.
"""
...
def build_prompt(term: str, shard_contexts: list[dict[str, Any]], max_items: int) -> str:
    ...
    return f"""你是 ISMISM W3 术语候选预处理低模型，只做 candidate-only evidence mining。
...
def write_shard_md(path: Path, term: str, batch_id: str, shard_no: int, contexts: list[dict[str, Any]], items: list[dict[str, Any]], model: str) -> None:
    lines = [
        f"# W3 Fullscan Candidate Shard — {term} / {shard_no:03d}",
        "",
        f"- term: {term}",
...
def write_summary(term_dir: Path, term: str, batch_id: str, contexts: list[dict[str, Any]], all_items: list[dict[str, Any]], shard_count: int) -> None:
    ...
    lines = [
        f"# W3 Fullscan Candidate Summary — {term}",
        "",
        f"- batch_candidate: {batch_id}",
        "- status: candidate-only",
        "- do_not_import_directly: true",
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Heavy full-corpus W3 candidate scan with local Ollama.")
    ap.add_argument("--repo", default=".")
    ap.add_argument("--batch-id", default="W3-fullscan-gemma4")
    ...
    root = repo / "knowledge/lexicon/candidates/fullscan" / args.batch_id
