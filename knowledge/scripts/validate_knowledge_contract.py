#!/usr/bin/env python3
"""Validate the repo-local ISMISM project knowledge contract.

This read-only validator complements the W3/W4/W5 validators. It checks the
current in-repository deliverables for corpus-grounded knowledge-layer readiness
and the active-surface forbidden-language rule. It has no external repository
dependency.
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Validate repo-local ISMISM project knowledge contract.")
    ...
    check_exists(repo, ("PROJECT-SPEC.md", "knowledge/STATE.md", "ISMISM-MAINLINE-HANDOFF.md"), errors)
    summary["project_spec"] = (repo / "PROJECT-SPEC.md").exists()
    ...
    if args.json:
        ...
        print(
            "ISMISM project contract validation: "
            ...
            f"project_spec={summary.get('project_spec')}, "
            f"errors={len(errors)}"
        )
